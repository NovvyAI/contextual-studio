...
async function generateGroup(sessionId, storyboard, placeholders, referenceSources) {
  try {
    ...
    const results = await Promise.allSettled(placeholders.map(async (card, index) => {
      ...
      const prompt = `Create storyboard frame ${index + 1} of ${placeholders.length} for a vertical 9:16 live-action mobile game advertisement. Storyboard concept: ${storyboard.title}. Overall story: ${storyboard.summary}. This shot: ${card.summary}. Approved shot contract:\n${shotContract}\nTreat the shot contract as binding for narrative function, start/peak/result state, character blocking and performance, shot size, composition, camera height and angle, lens perspective, depth and focus, motivated camera movement endpoint, lighting and color continuity, gameplay action, object positions, and transition. Preserve the exact identities, faces, ages, hairstyles, costumes, and accessories from the supplied character references. The still frame should depict the most informative action peak or result state and make the physical cause and visible result readable without captions. Compose one production-ready cinematic key frame, not a collage, contact sheet, split screen, captioned storyboard, or UI mockup. No labels, subtitles, watermarks, borders, shot numbers, or explanatory text. Do not use a transition, crop, blur, flare, or UI overlay to hide identity, hand, prop, or spatial continuity problems.`;
