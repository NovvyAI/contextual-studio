...
export async function approveAndFinalizeVideoShots(sessionId, promptCardId) {
  ...
  const promptCard = latestCard(sessionId, promptCardId);
...
export function approveFinalVideo(sessionId, promptCardId) {
  const session = db.prepare("SELECT * FROM creative_sessions WHERE id=?").get(sessionId);
  if (!session) throw new Error("创意工作台不存在");
  const card = latestCard(sessionId, promptCardId);
  if (!card || card.kind !== "video_prompt" || !card.previewUrl) throw new Error("找不到可确认的最终成片");
  ...
  const confirmedCard = { ...card, status: "confirmed", confirmedAt: timestamp, details: (card.details || []).filter((item) => item.label !== "失败原因") };
