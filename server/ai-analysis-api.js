...
export function adaptDramaAnalysis(payload) {
  ...
  // server/drama-analysis-v3.js's dramaAnalysisView() reads `episodeIndex` (not
  // `episodeNumber`) as the canonical per-episode ordinal, and for multi-episode
  // dramas prefers a top-level `seriesAnalysis.synopsis` over joining per-episode
  // synopses. Both are supplied below so the remote projection renders identically
  // to a locally-analyzed multi-episode drama instead of falling back to a
  // "第undefined集" join.
  ...
  return {
    ...
    episodeAnalyses,
    seriesAnalysis: episodeAnalyses.length > 1 ? {
      synopsis: synopsis || episodeAnalyses.map((item) => `第${item.episodeIndex}集：${item.detailedAnalysis?.synopsis || ""}`).join("\n"),
