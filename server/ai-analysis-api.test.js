...
test("adapts AI Analysis API drama results without discarding remote evidence", () => {
  ...
  // dramaAnalysisView() (server/drama-analysis-v3.js) is the same projection a
  // locally-analyzed multi-episode drama goes through; a remote drama must render
  // through it identically, keyed by episodeIndex rather than episodeNumber.
  const analysisView = dramaAnalysisView(result);
  assert.equal(analysisView.synopsis, "女主被迫代嫁后争取自己的生活。");
  assert.equal(result.episodeAnalyses.every((item) => item.episodeIndex === item.episodeNumber), true);
  assert.match(analysisView.chronology[0].timeRange, /^第1集/);
