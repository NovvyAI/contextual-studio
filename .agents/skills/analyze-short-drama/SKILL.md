...
2. Build visual coverage.
   ...
   - In Contextual Studio, run the project-local `scripts/detect_face_candidates.py` pipeline for character-reference discovery. Treat its output as optional anonymous face observations, not the authority on how many characters exist. Build the canonical character list from narrative evidence, include every visually distinct main, co-lead, supporting, antagonistic, non-human, masked, or gender-unknown character, and allow any number or gender composition. For every narrative character, retain 1-3 overview-frame fallbacks so a YuNet miss never removes that character from the dynamic library. Keep uncertain face observations unassigned rather than forcing male/female protagonist slots.
