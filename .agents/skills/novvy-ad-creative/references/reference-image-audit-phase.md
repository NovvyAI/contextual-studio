...
远程 URL 不是稳定审核素材。所有准备进入上传计划的远程图都先用允许的下载脚本物化到 Novvy workspace，再查看该本地副本，并让 `source` 指向这份已审核的本地文件。禁止审核一个 URL 后让平台重新拉取可能变化的另一份内容。
...
`asset` 模式若发现任何 `human_photorealistic + pass`，不要继续普通上传；输出模式不匹配，让父流程改走 `seedance-human`。`mixed_unknown` 不能把不确定人物当真人，也不能把已确认真人悄悄放进 `imageUrls`。
...
```json
{
  ...
  "auditId": "",
  "uploadMode": "asset|seedance-human",
  "requiredSlots": [],
  "waivedSlots": [],
  "acceptedSlots": [],
  "confirmedHumanSlots": [],
...
不要返回图片二进制、完整来源 URL、完整脚本输出或父流程长上下文。上传 subagent 从 `uploadPlanPath` 读取精确槽位、来源和哈希。
