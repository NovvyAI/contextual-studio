...
- 上传本地短剧视频并保存历史记录。
- 使用 Codex SDK 和项目内 `$analyze-short-drama` Skill：支持单个视频或包含多集视频的文件夹；每集只分析前 900 秒，每 5 秒截取一帧、每 20 帧生成一张连续概览拼图，一次纯视觉模型分析后持久化逐集结果；不读取或转写音频。独立截图仍保存到数据库，供人物候选和资产区使用。
